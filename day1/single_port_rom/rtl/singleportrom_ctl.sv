...
///////////////////////////////////////////
/*
    IDLE -- (start) => READ --(addr == 99)=>WAIT_DATA --1CLK --> WAIT_DONE => DONE
    */
typedef enum logic [2:0] {IDLE,READ,WAIT_DATA,WAIT_DONE,DONE} state_t;
state_t state;

assign start_pulse = start_r & ~start_r_d;
assign rom_en      = (state == READ) || ((state == IDLE) && start_pulse) || (state == WAIT_DATA);
...
always_ff @(posedge clk or negedge rst_n) begin
    if (!rst_n) state <= IDLE;
        ...
        case (state)
            ...
            READ:       if (rom_addr == 7'd99)  state <= WAIT_DATA;
            WAIT_DATA:                          state <= WAIT_DONE;
